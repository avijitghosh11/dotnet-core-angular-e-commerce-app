...
export class CartService {
  ...
  async addItemToCart(item: CartItem | Product, quentity = 1) {
    const cart = this.cart() ?? this.createCart();
    if (this.isProduct(item)) {
      item = this.mapProductToCartItem(item);
    }
    cart.items = this.addOrUpdateItem(cart.items, item, quentity);
    await firstValueFrom(this.setCart(cart));
  }

  private addOrUpdateItem(items: CartItem[], item: CartItem, quentity: number): CartItem[] {
