...
namespace Ekart.Api.Controllers
{
    public class PaymentsController(IPaymentService paymentService,
     ...
     IConfiguration config, IHubContext<NotificationHub> hubContext, IWebHostEnvironment environment) : BaseApiController
    {
        ...
        private Event ConstructStripeEvent(string json)
        {
            try
            {
                string whSecret = _whSecret;
                if (environment.IsEnvironment("development"))
                {
                    whSecret = Environment.GetEnvironmentVariable("StripeSettings_WhSecret").ToString();
                }
